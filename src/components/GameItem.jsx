        ...
        <>
            ...
            <div className="card w-80 h-96 bg-base-300 shadow-xl rounded-2xl p-4 hover:scale-105 transition">
                <figure className="h-48 relative shadow-lg">
                    ...
                    {game.isPCOnly && <div className="absolute top-0 right-0 text-3xl text-black z-10">📵</div>}
                </figure>
                <div className="card-body items-center text-center">
                    <h2 className="card-title text-xl font-semibold arcade-font">{game.title}</h2>
                    <p>{game.summary}</p>
                    <div className="card-actions  justify-end">
                        <Link
                            to={game.link}
                            className="self-end">
                            {game.link ? <button className="btn btn-primary">Play ▶</button> : <button className="btn btn-disabled ">Under Development</button>}
                        </Link>
                    </div>
                </div>
            </div>

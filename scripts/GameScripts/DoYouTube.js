...
function getFetchUrl(vidcat) {
  let url =
    ...
    vidcat +
    "&key=AIzaSyALoTeC4RHeVkkVAkkDdYxKrRZxDODjKGQ";
  //console.log(url)
  return url;
}
...
function startGame() {
  ...
  categoryNameText.innerHTML =
    "Category : " + document.querySelector(".active").innerText;
}
...
function setLeftHalf(i) {
  document.documentElement.style.setProperty(
    "--leftUrl",
    `url(${videoInfo[i].snippet.thumbnails.standard.url}`
  );
  leftHalf.innerHTML = `
    <div class="half">
        <p class = "vidTitle">${videoInfo[i].snippet.title}</p>
        <p class="author">By : ${videoInfo[i].snippet.channelTitle}</p>
        <img width=640px height=480px src="${
          videoInfo[i].snippet.thumbnails.standard.url
        }"/>
        <p>has</p>
...
async function getVideoInfo() {
  ...
  return fetch(
    getFetchUrl(
      videoCategories.get(document.querySelector(".active").innerText)
    )
  )
    .then((result) => {
      return result.json();
...
async function asyncCall() {
  ...
  rightHalf.classList.toggle("a2");
  roundNum++;
  roundNumText.innerHTML =
    "Round Number : " + roundNum + "/" + (videoInfo.length - 1);
